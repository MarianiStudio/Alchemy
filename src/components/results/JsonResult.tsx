...
import { prettyPrintJson, minifyJson, jsonToTypeScript } from '../../lib/transformers';
...
export function JsonResult({ data, delay = 0 }: JsonResultProps) {
    const { t } = useI18n();
    const [copiedId, setCopiedId] = useState<string | null>(null);

    const pretty = prettyPrintJson(data);
    const minified = minifyJson(data);
    const typescript = jsonToTypeScript(data);
    ...
    const results = [
        { id: 'pretty', label: t('json.title'), content: pretty, language: 'json' },
        { id: 'minified', label: t('json.minified'), content: minified, language: 'json' },
        { id: 'typescript', label: t('json.typescript'), content: typescript, language: 'typescript' },
    ];
